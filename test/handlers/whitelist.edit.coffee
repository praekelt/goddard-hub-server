  ...
  describe '/whitelist', ->
    ...
    describe '#response', ->
      ...
      it 'should redirect when trying to GET node that doesnt exist', ->
        ...
        request(app)
          .get('/whitelist/2?logged_in_user_id=1')
          .expect(302)
          .end((err, res)->
            assert(err == null, 'Was not expecting a error after request')
          )
      ...
      it 'should show the name of the node', ->
        ...
        request(app)
          .get('/whitelist/1?logged_in_user_id=1')
          .expect(200)
          .end((err, res)->
      ...
      it 'should update the name of the node when posting', ->
        ...
        request(app)
          .post('/whitelist/1?logged_in_user_id=1')
          ...
          .end((err, res)->
            ...
            app.get('models').whitelist.find(1).then((node_obj) ->

              # check the returned user obj
              assert(node_obj != null, 'was expecting a whitelist obj ...')
