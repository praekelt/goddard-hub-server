...
describe 'Handlers', ->
	...
	describe '/groups', ->
		...
		describe '#response', ->
			...
			# handle the error output
			it 'should redirect when trying to GET group that doesnt exist', ->

				request(app)
					.get('/groups/2?logged_in_user_id=1')
					.expect(302)
					.end((err, res)->
						assert(err == null, 'Was not expecting a error after request')
					)
			...
			# handle the error output
			it 'should show the name of the group', ->
			...
			# handle the error output
			it 'should update the name of the group when posting', ->

				request(app)
					.post('/groups/1?logged_in_user_id=1')
					.send({ name: 'test2' })
					.expect(302)
					.end((err, res)->
						...
						app.get('models').groups.find(1).then((group_obj) ->

							# check the returned user obj
							assert(group_obj != null, 'was expecting a groups obj ...')

							# check the 
							assert(group_obj.name == 'test2', 'Name must be test2 after the update')
