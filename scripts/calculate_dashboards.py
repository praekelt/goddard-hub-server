...
def upsert(cursor, nodeid, stat):
    cursor.execute('SELECT id FROM node_dashboard_page_view WHERE "nodeId"=%s', (node_id,))
    if cursor.fetchone() is None:
        # Record does not yet exist, lets add it. 
        print "Dashboard Calculations - Inserting node %s" % node_id
        cursor.execute('INSERT INTO node_dashboard_page_view ("nodeId", "h1", "h24", "h48", "d7", "d31", "d365", "allTime") \
        ...
        cursor.execute('UPDATE node_dashboard_page_view set "h1"=%s, "h24"=%s, "h48"=%s, "d7"=%s, "d31"=%s, "d365"=%s, \
            "allTime"=%s, timestamp=now() where "nodeId" = %s',
            (stat['h1'], stat['h24'], stat['h48'], stat['d7'], stat['d31'], stat['d365'], stat['all_time'], node_id, ))


def get_sum(cursor, node_id, target):
    cursor.execute('SELECT SUM("pagesServed") FROM node_access WHERE "nodeId"=%s AND "hourLoggedAt" >= %s', (node_id, target, ))
    r = cursor.fetchone()
...
def calculate_page_views(node_id, cursor):
    ...
    stats[node_id]['h1'] = get_sum(cursor,node_id,t)
    
    # 24 hours
    t = now - timedelta(hours=24)
    stats[node_id]['h24'] = get_sum(cursor,node_id,t)
    
    # 48 hours
    t = now - timedelta(hours=48)
    stats[node_id]['h48'] = get_sum(cursor,node_id,t)    
    
    # 7 days
    t = now - timedelta(days=7)
    stats[node_id]['d7'] = get_sum(cursor,node_id,t) 
    
    # 31 days
    t = now - timedelta(days=31)
    stats[node_id]['d31'] = get_sum(cursor,node_id,t) 

    # 365 days
    t = now - timedelta(days=365)
    stats[node_id]['d365'] = get_sum(cursor,node_id,t) 

    # all time 
    t = date(1970, 1, 1)
    stats[node_id]['all_time'] = get_sum(cursor,node_id,t)
...
if __name__ == '__main__':
    ...
    for row in rows:
        calculate_page_views(row['id'], cursor)
